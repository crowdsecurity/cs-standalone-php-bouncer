...
```bash
	mkdir -p /var/www/html/crowdsec-standalone-bouncer
```

We use here `/var/www/html/crowdsec-standalone-bouncer` but you can choose the path that suits your needs.

- Retrieve the last version of the bouncer:

```bash
composer create-project crowdsec/standalone-bouncer /var/www/html/crowdsec-standalone-bouncer --keep-vcs
```
...
The owner of the `/var/www/html/crowdsec-standalone-bouncer` folder should be your web-server owner (e.g. `www-data`).

You can achieve it by running command like:

```bash
sudo chown www-data /var/www/html/crowdsec-standalone-bouncer
```
...
You should add this line to a `.ini` file :
    ...
    auto_prepend_file = /var/www/html/crowdsec-standalone-bouncer/scripts/bounce.php
...
location ~ \.php$ {
    ...
    ...
    ...
    fastcgi_param PHP_VALUE "/var/www/html/crowdsec-standalone-bouncer/scripts/bounce.php";
}
```
...
If you are using Apache, you should add this line to your `.htaccess` file:
    ...
    php_value auto_prepend_file "/var/www/html/crowdsec-standalone-bouncer/scripts/bounce.php"
...
```shell
*/15 * * * * /usr/bin/php /var/www/html/crowdsec-standalone-bouncer/scripts/refresh-cache.php
```
...
```shell
0 0 * * * /usr/bin/php /var/www/html/crowdsec-standalone-bouncer/scripts/prune-cache.php
```
...
Once the update is done, you can reactivate the bounce. You could look at the `/var/www/html/crowdsec-standalone-bouncer/scripts/.logs` to see if all is working as expected.
...
```bash
cd /var/www/html/crowdsec-standalone-bouncer
git fetch
```
